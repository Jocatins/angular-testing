...
export class ProductsService {
  ...
  getAllProducts(): Observable<Product> {
    const productUrl = 'http://localhost:3600/products';

    return this.httpClient.get<Product>(productUrl); // return an observable
  }

  getCategories(): Observable<Category> {
    const categoriesUrl = 'http://localhost:3600/categories';
  ...
  viewProduct(productId): Observable<Product> {
    const productUrl = 'http://localhost:3600/products/' + productId;
    return this.httpClient.get<Product>(productUrl); // return an observable
  }
  ...
  searchCategoryProducts(categoryId): Observable<Product> {
    const productUrl =
      'http://localhost:3600/products?categoryId=' + categoryId;
    return this.httpClient.get<Product>(productUrl); // return an observable
  }

  searchDateProducts(dateParam): Observable<Product> {
    const productUrl = 'http://localhost:3600/products/date=' + dateParam;
    return this.httpClient.get<Product>(productUrl); // return an observable
  }
}
